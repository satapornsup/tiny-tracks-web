...
### 3.7 หน้า Start — responsive sizing ต้อง "หน้าตาเหมือนกัน" ระหว่างจอ 1920/1440 (confirm กับ user 2026-08-22)

**ข้อกำหนด**: ของตกแต่ง + โลโก้ + ปุ่ม START ในหน้า Start ถูก tune ขนาด (`px`) ด้วยตาบนจอใหญ่จนพอใจแล้ว — ต้องการให้ **จอ 1920px กับ 1440px ให้ผลลัพธ์หน้าตาเหมือนกัน/สัดส่วนตรงกัน** เวลาเอาไปโชว์ (เดโม/พรีเซนต์คนละเครื่อง คนละจอ ต้องไม่ดูต่างกัน) — ส่วนจอที่ **เล็กกว่า 1440px ค่อยๆ หดลงได้ ไม่ต้องเท่ากันเป๊ะ** (มือถือ/แท็บเล็ตหดเล็กลงเป็นเรื่องปกติ)

**บั๊กที่เจอรอบแรก**: ใช้ `width: min(Npx, Mvw)` (px สูงสุด + vw ตัวคุมการหด) แต่พอผู้ใช้ปรับ `px` (max) ให้ใหญ่ขึ้นทีหลัง ไม่ได้ปรับ `vw` ตาม ทำให้ที่จอ desktop ทั่วไป (~1300-1900px) `vw` แพ้ `px` ตลอด องค์ประกอบเลย **ค้างที่ขนาด max เท่ากันหมดตั้งแต่ ~1280px ขึ้นไป** — จอ 1920 กับ 1440 (ทั้งคู่เกิน 1280) เลยออกมา**เหมือนกันโดยบังเอิญที่ค้าง** ไม่ใช่เพราะ tune ให้ตรงกันจริง แล้วพอลดจอลงต่ำกว่า breakpoint 640px ก็ "กระโดด" ไปใช้ค่า mobile แยกทันที ไม่ลื่นไหล

**ทางแก้**: เปลี่ยนทุกจุด (`&__decor--*`, `&__logo`, `&__cta`) จาก `min(px, vw)` เป็น **`clamp(floor_px, K vw, ceiling_px)`** ตัวเดียว โดย

```
K = (ceiling_px ÷ 1920) × 100
```

ใช้ **1920px เป็นจอ reference** (ไม่ใช่ 1280) เพราะ user tune ค่าบนจอใหญ่ — แปลว่าที่ 1920px ขนาดจะเท่า `ceiling_px` (max) พอดี แล้วหดตาม `vw` ต่อเนื่องลงมาเรื่อยๆ จนถึง `floor_px` (ค่า mobile เดิมที่เคย tune ไว้) ไม่มีจุดค้าง ไม่มีจุดกระโดดที่ 640px อีก (`floor`/`ceiling` เป็นแค่ขอบเขตบนล่าง ไม่ใช่ breakpoint)

**ผลลัพธ์ที่ verify แล้ว**: 1920px → logo = 800px (ceiling เต็ม), 1440px → logo ≈ 600px (หดตามสัดส่วน `vw` จริง ไม่ใช่ค้าง) — ทั้งสองจอเลยดู "สัดส่วนเดียวกัน" ตามที่ user ต้องการ (composition เดียวกัน ขนาดสัมพัทธ์เท่ากัน แค่ scale ต่างกันตามจอ ไม่ใช่หน้าตาต่างกัน) โค้ดอ้างอิง: `src/app/features/start/start.component.scss`

**ถ้าจะปรับขนาด (`ceiling_px`) อีกในอนาคต — ต้องคำนวณ `K` ใหม่ทุกครั้งด้วยสูตรข้างบน** ห้ามแก้แค่ `px` เฉยๆ แล้วปล่อย `vw` ค้างไว้เหมือนบั๊กรอบแรก (ถ้าจะเปลี่ยน reference viewport จาก 1920 เป็นค่าอื่น ก็ต้องคำนวณ `K` ใหม่หมดทุกจุดด้วยสูตรเดียวกัน ไม่ใช่แค่จุดที่เพิ่งแก้)
